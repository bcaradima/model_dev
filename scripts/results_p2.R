...
k <- 3
# Cut the randomly shuffled data into k-folds
k.folds <- cut(seq(1, nrow(sample.invf.plat)), breaks=k, labels=FALSE)

fold1 <- sample.invf.plat[which(k.folds == 1),]
fold2 <- sample.invf.plat[which(k.folds == 2),]
fold3 <- sample.invf.plat[which(k.folds == 3),]
...
write.csv(sample.invf.plat, 'outputs/inv.family.plateau.sample.csv', row.names = F)
