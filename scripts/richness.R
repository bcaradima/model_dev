...
# rdt.pred$Rank[rdt.pred$Variable=="PredRareFamilies"] <- "Rare families"
# rdt.pred$Rank[rdt.pred$Variable=="PredRareSpecies"] <- "Rare species"
...
# rdt.obs$Rank[rdt.obs$Variable=="ObsRareFamilies"] <- "Rare families"
# rdt.obs$Rank[rdt.obs$Variable=="ObsRareSpecies"] <- "Rare species"
...
# rdt.plot$Rank <- factor(rdt.plot$Rank, levels = c("Families", "Rare families", "Species", "Rare species"))

setDT(rdt.plot)
# lm.data <- data.table(RE.label = rep(c("Without random effect", "With random effect"), 2),
#                       Rank = c("Families", "Families", "Species", "Species"),
#                       Eqn = c(write_lm("Observed", "Predicted", rdt.plot[RE==F & Rank == "Families",]),
#                               write_lm("Observed", "Predicted", rdt.plot[RE==T & Rank == "Families",]),
#                               # write_lm("Observed", "Predicted", rdt.plot[RE==F & Rank == "Rare families",]),
#                               # write_lm("Observed", "Predicted", rdt.plot[RE==T & Rank == "Rare families",]),
#                               write_lm("Observed", "Predicted", rdt.plot[RE==F & Rank == "Species",]),
#                               write_lm("Observed", "Predicted", rdt.plot[RE==T & Rank == "Species",])),
#                       # write_lm("Observed", "Predicted", rdt.plot[RE==F & Rank == "Rare species",]),
#                       # write_lm("Observed", "Predicted", rdt.plot[RE==T & Rank == "Rare species",])),
#                       x = c(11,11 , 10,10),
#                       y = c(30,30 , 30,30)
# )
# lm.data$Rank <- factor(lm.data$Rank, levels = c("Families", "Species"))
# 
# 
# lm.data <- lm.data[Rank == 'Families' | Rank == 'Species', ]
# lm.data$Rank <- droplevels(lm.data$Rank)
...
# Richness model ####
p1.predictors <- prepare.inputs(c("Temp","Temp2", "FV", "F10m", "IAR", "Urban", "LUD"), y = sample.bdms, center = TRUE)
rm.data <- rdt %>%
  filter(RE == FALSE) %>%
  select(-PredFamilies, -PredSpecies) %>% # Remove joint model results
  left_join(select(p1.predictors,-SiteId),by="SampId") %>%
  as.tibble()
...
#          Type = "Predicted",
#          Rank = "Species") %>%
#   select(SampId, Fraction, Richness, Type, Rank, RE)
# 
# prare.os <- rdt %>%
#   mutate(Fraction = ObsRareSpecies/ObsSpecies,
#          Richness = ObsSpecies,
...
#   select(SampId, Fraction, Richness, Type, Rank, RE)
# 
# data <- bind_rows(prare.ps, prare.os, prare.pf, prare.of)
# data$RE.label <- ifelse(data$RE, "Random effect: TRUE", "Random effect: FALSE")
# 
# lm.data <- data.table(Rank = c("Family", "Family", "Species", "Species"),
#                       RE.label = rep(c("Random effect: TRUE", "Random effect: FALSE"), 2),
#                       Eqn = c(write_lm("Richness", "Fraction", filter(data, Rank == "Family" & RE == TRUE)),
#                               write_lm("Richness", "Fraction", filter(data, Rank == "Family" & RE == FALSE)),
#                               write_lm("Richness", "Fraction", filter(data, Rank == "Species" & RE == TRUE)),
#                               write_lm("Richness", "Fraction", filter(data, Rank == "Species" & RE == FALSE))),
#                       x = rep(10, 4),
#                       y = rep(0.85, 4)
# )
# g <- ggplot(data, aes(x = Richness, y = Fraction))
# g <- g + geom_point(alpha = 0.35)
# g <- g + geom_smooth(method='lm')
# g <- g + geom_text(data=lm.data, aes(x = x, y = y, label = Eqn), parse = TRUE, inherit.aes=FALSE)
# g <- g + facet_grid(RE.label ~ Rank)
# g <- g + theme_bw(base_size = 17)
# g <- g + labs(title = "Proportion of predicted rare taxa vs observed sample richness",
#               subtitle = "Joint model with and without random effect",
#               x = "Observed richness",
#               y = "Predicted proportion of rare taxa")
# print(g)
