# > RICHNESS ####
# Description: this script prepares predicted and observed richness at family/species-level for various models: individual, joint, or negative-binomial GLM. Prepares the plots as well.
...
rm.data$PredFamilies.RM <- exp(predict(rmf))
rm.data$PredSpecies.RM <- exp(predict(rms))

rm <- rm.data %>%
  select(SampId, PredFamilies.RM, ObsFamilies, PredSpecies.RM, ObsSpecies) %>%
  gather(Variable, Value, -SampId)

rm$Rank[rm$Variable=="PredFamilies.RM"] <- "All families"
rm$Rank[rm$Variable=="ObsFamilies"] <- "All families"
rm$Rank[rm$Variable=="PredSpecies.RM"] <- "EPT species"
rm$Rank[rm$Variable=="ObsSpecies"] <- "EPT species"


rm$Variable[rm$Variable=="PredFamilies.RM"] <- "Predicted"
rm$Variable[rm$Variable=="ObsFamilies"] <- "Observed"
rm$Variable[rm$Variable=="PredSpecies.RM"] <- "Predicted"
...
dt <- rdt %>%
  ...
  left_join(select(predictors.p1, -SiteId), by="SampId") %>%
  as.tibble()
rmse(dt$ObsFamilies-dt$PredFamilies)
...
# Species
# Negative binomial GLM
rmse(rm.data$ObsSpecies-exp(predict(rms)))
# Joint model
rmse(dt$ObsSpecies-dt$PredSpecies)
